...
gkernel::Segment pointintersection(const gkernel::Segment &a, const gkernel::Segment &b){
    ...
    if ((a1 * b2 - a2 * b1) != 0) {
        float x = (float)(b1 * c2 - b2 * c1) / (float)(a1 * b2 - a2 * b1);
        float y = (float)(a2 * c1 - a1 * c2) / (float)(a1 * b2 - a2 * b1);
        return gkernel::Segment({ x, y }, { x, y });
    } else {
        if (a.begin_point().x() <= b.begin_point().x() && b.end_point().x() <= a.end_point().x()) {
            return gkernel::Segment({ b.begin_point().x(), b.begin_point().y() }, { b.end_point().x(), b.end_point().y() });
        } else if (a.begin_point().x() <= b.begin_point().x() && a.end_point().x() <= b.end_point().x()) {
                    return gkernel::Segment({ b.begin_point().x(), b.begin_point().y() }, { a.end_point().x(), a.end_point().y() });
        } else if (b.begin_point().x() <= a.begin_point().x() && a.end_point().x() <= b.end_point().x()) {
                    return gkernel::Segment({ a.begin_point().x(), a.begin_point().y() }, { a.end_point().x(), a.end_point().y() });
        } else if (b.begin_point().x() <= a.begin_point().x() && b.end_point().x() <= a.end_point().x()) {
                    return gkernel::Segment({ a.begin_point().x(), a.begin_point().y() }, { b.end_point().x(), b.end_point().y() });
        }
    }
}
