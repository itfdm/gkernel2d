...
bool intersect_parallel(gkernel::Segment &a, gkernel::Segment &b) {
    int A1 = a.begin_point().y() - a.end_point().y(),
        B1 = a.end_point().x() - a.begin_point().x(),
        C1 = -A1 * a.begin_point().x() - B1 * a.begin_point().y();
    int A2 = b.begin_point().y() - b.end_point().y(),
        B2 = b.end_point().x() - b.begin_point().x(),
        C2 = -A2 * b.begin_point().x() - B2 * b.begin_point().y();
    int zn = det(A1, B1, A2, B2);
    if (zn != 0) {
        double x = -det(C1, B1, C2, B2) * 1. / zn;
        double y = -det(A1, C1, A2, C2) * 1. / zn;
        return between(a.begin_point().x(), a.end_point().x(), x) &&
               between(a.begin_point().y(), a.end_point().y(), y) &&
               between(b.begin_point().x(), b.end_point().x(), x) &&
               between(b.begin_point().y(), b.end_point().y(), y);
    } else
...
struct event {
    ...
    event() {}
    event(gkernel::data_type x, int tp, int id) : x(x), tp(tp), id(id) {}
